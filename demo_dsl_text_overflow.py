...
Run:
    python demo_dsl_text_overflow.py
...
def normalize_overflow(value: Any, default: str = "wrap") -> str:
    ...
    if mode not in {"wrap", "shrink", "ellipsis", "clip"}:
        return default
    return mode
                ...
                for item in obj["items"]:
                    ...
                    if icon:
                        parts.append(
                            ...
                            f"{icon}</span><span style='line-height:1.25; flex:1; min-width:0; overflow-wrap:anywhere;'>{text}</span></div>"
                        )
                    else:
