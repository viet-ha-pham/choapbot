#!/usr/bin/env python3
"""
Legal/article-first long-report slide pipeline with real LLM + ontology consultation
========================================================
...
- Every `Điều N`/`PHỤ LỤC` becomes a source unit.
- If a unit is too long, it is chunked, each chunk is summarized by LLM, then merged by LLM.
- Every source unit is classified using Report Document Ontology, mapped to a presentation intent using Presentation Intent Ontology, and assigned a renderer-supported layout using Layout Registry.
...
def load_json_file(path: str | Path | None, default: Any = None) -> Any:
    """Load a JSON knowledge source; return default if path is empty/missing."""
    if not path:
        return default
    p = Path(path)
    if not p.exists():
...
def _norm_key_text(text: Any) -> str:
    s = str(text or "").lower()
    s = re.sub(r"[\W_]+", " ", s, flags=re.UNICODE)
    return re.sub(r"\s+", " ", s).strip()


def _keyword_score(haystack: str, labels: List[str]) -> int:
    h = _norm_key_text(haystack)
    score = 0
    for label in labels or []:
        lab = _norm_key_text(label)
        if lab and lab in h:
            score += max(1, len(lab.split()))
    return score


def query_report_ontology(unit: Dict[str, Any], ontology: Dict[str, Any] | None, logger: logging.Logger | None = None) -> Dict[str, Any]:
    """Classify a real source unit using Report Document Ontology.

    This does not create or delete source units. It only attaches a concept label
    and confidence/evidence to an existing unit extracted from the document.
    """
    title = unit.get("title", "")
    content = unit.get("content", "")
    kind = unit.get("kind", "")
    search_text = f"{unit.get('numbering','')} {title} {content[:1200]}"
    concepts = (ontology or {}).get("concepts", []) if isinstance(ontology, dict) else []

    best: Dict[str, Any] = {
        "concept_id": "legal_article" if kind == "article" else ("appendix_table" if kind == "appendix" else "general_content"),
        "confidence": 0.25,
        "evidence": "fallback_by_unit_kind",
        "matched_labels": [],
    }
    best_score = 0
    for concept in concepts:
        labels = concept.get("labels") or concept.get("aliases") or []
        score = _keyword_score(search_text, labels)
        if score > best_score:
            best_score = score
            matched = [lab for lab in labels if _norm_key_text(lab) in _norm_key_text(search_text)]
            best = {
                "concept_id": concept.get("id", "unknown"),
                "confidence": min(0.95, 0.45 + 0.10 * score),
                "evidence": "keyword_alias_match",
    ...
    if logger:
        logger.info(
            "[ONTOLOGY REPORT] unit=%s title=%s concept=%s confidence=%.2f evidence=%s",
            unit.get("id"), title, best.get("concept_id"), best.get("confidence", 0), best.get("evidence"),
        )
    return best


def query_presentation_ontology(unit: Dict[str, Any], report_result: Dict[str, Any], ontology: Dict[str, Any] | None, logger: logging.Logger | None = None) -> Dict[str, Any]:
    """Infer presentation intent using Presentation Intent Ontology.

    It uses the classified report concept plus source title/content signals.
    """
    fallback = (ontology or {}).get("fallback_intent", "summarize_core_message") if isinstance(ontology, dict) else "summarize_core_message"
    intents = (ontology or {}).get("intents", []) if isinstance(ontology, dict) else []
    search_text = " ".join([
        str(unit.get("title", "")),
        str(unit.get("numbering", "")),
        str(report_result.get("concept_id", "")),
        str(unit.get("content", ""))[:1600],
    ])
    if unit.get("kind") == "appendix":
        search_text += " số liệu chỉ tiêu mức thu kinh phí bảng phụ lục"

    best = {"intent_id": fallback, "confidence": 0.35, "evidence": "fallback_intent", "matched_labels": []}
    best_score = 0
    for intent in intents:
        labels = intent.get("labels") or []
        score = _keyword_score(search_text, labels)
        # concept-to-intent hints for common report concepts
        cid = report_result.get("concept_id", "")
        if cid in {"implementation_results", "highlights"} and intent.get("id") == "present_findings":
            score += 3
        if cid in {"limitations_causes", "warnings_risks"} and intent.get("id") == "highlight_risks":
            score += 3
        if cid in {"measures_solutions", "proposals_recommendations"} and intent.get("id") == "recommend_actions":
            score += 3
        if cid in {"tasks_assignment"} and intent.get("id") == "assign_responsibilities":
            score += 3
        if cid in {"implementation_organization"} and intent.get("id") == "organize_implementation":
            score += 3
        if cid in {"timeline_progress"} and intent.get("id") == "show_progression":
            score += 3
        if cid in {"data_metrics", "appendix_table"} and intent.get("id") == "show_metrics":
            score += 3
        if score > best_score:
            best_score = score
            matched = [lab for lab in labels if _norm_key_text(lab) in _norm_key_text(search_text)]
            best = {
                ...
                "confidence": min(0.95, 0.40 + 0.10 * score),
                "evidence": "ontology_signal_match",
                "matched_labels": matched[:5],
                "description": intent.get("description", ""),
                "preferred_content_shape": intent.get("preferred_content_shape", ""),
            }

    if logger:
        logger.info(
            "[ONTOLOGY PRESENTATION] unit=%s intent=%s confidence=%.2f evidence=%s",
            unit.get("id"), best.get("intent_id"), best.get("confidence", 0), best.get("evidence"),
        )
    return best


def query_layout_registry(unit: Dict[str, Any], intent_result: Dict[str, Any], registry: Dict[str, Any] | None, logger: logging.Logger | None = None) -> Dict[str, Any]:
    """Choose a renderer-supported layout from Layout Registry.

    The current PML writer is bullet-oriented, so this selector prefers layouts
    that support title+bullets unless the registry has no such candidate.
    """
    layouts = (registry or {}).get("layouts", []) if isinstance(registry, dict) else []
    intent_id = intent_result.get("intent_id", "summarize_core_message")
    available_blocks = {"title", "subtitle", "bullets", "conclusion"}
    bullet_count_hint = 6
    scored: List[Dict[str, Any]] = []
    for layout in layouts:
        lid = layout.get("id", "")
        intents = layout.get("intents") or layout.get("supported_intents") or []
        supported_blocks = set(layout.get("supported_blocks") or layout.get("required_blocks") or [])
        capacity = layout.get("capacity", {}) or {}
        score = 0.0
        if intent_id in intents:
            score += 5.0
        if "bullets" in supported_blocks:
            score += 2.0
        if supported_blocks and supported_blocks.issubset(available_blocks):
            score += 1.5
        if lid == "title-bullets":
            score += 1.0
        max_items = capacity.get("max_items") or capacity.get("max_cells") or capacity.get("max_columns")
        if isinstance(max_items, int) and max_items < bullet_count_hint:
            score -= 1.0
        if score > 0:
            scored.append({"layout_id": lid, "score": round(score, 3), "description": layout.get("description", ""), "supported_blocks": list(supported_blocks), "intents": intents})

    scored.sort(key=lambda x: x["score"], reverse=True)
    chosen = scored[0] if scored else {"layout_id": "title-bullets", "score": 0.1, "description": "fallback safe bullet layout"}
    if chosen.get("layout_id") != "title-bullets":
        # Until the PML writer can reliably emit all data shapes, use safe layout but retain recommendation.
        chosen = {**chosen, "recommended_layout_id": chosen.get("layout_id"), "layout_id": "title-bullets", "reason": "safe_pml_writer_supports_title_bullets"}
    if logger:
        logger.info(
            "[LAYOUT REGISTRY] unit=%s intent=%s selected=%s recommendation=%s score=%s",
            unit.get("id"), intent_id, chosen.get("layout_id"), chosen.get("recommended_layout_id", chosen.get("layout_id")), chosen.get("score"),
        )
    return {"selected_layout": chosen.get("layout_id", "title-bullets"), "candidates": scored[:5], **chosen}


def consult_knowledge_sources(
    units: List[Dict[str, Any]],
    report_ontology: Dict[str, Any] | None,
    presentation_ontology: Dict[str, Any] | None,
    layout_registry: Dict[str, Any] | None,
    logger: logging.Logger | None = None,
) -> List[Dict[str, Any]]:
    """Attach ontology/registry consultation results to every source unit."""
    enriched = []
    for unit in units:
        u = dict(unit)
        report = query_report_ontology(u, report_ontology, logger)
        intent = query_presentation_ontology(u, report, presentation_ontology, logger)
        layout = query_layout_registry(u, intent, layout_registry, logger)
        u["knowledge"] = {
            "report_ontology": report,
            "presentation_ontology": intent,
            "layout_registry": layout,
        }
        u["report_concept"] = report.get("concept_id")
        u["presentation_intent"] = intent.get("intent_id")
        u["selected_layout"] = layout.get("selected_layout", "title-bullets")
        enriched.append(u)
    return enriched
...
def slide_spec_from_unit(unit: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "status": "completed",
        "title": slide_title,
        "intent": unit.get("presentation_intent") or ("summarize_legal_article" if unit.get("kind") == "article" else "summarize_appendix_table"),
        "layout": unit.get("selected_layout") or "title-bullets",
...
def build_task_prompt(unit: Dict[str, Any], max_bullets: int = 7) -> str:
    ...
    ontology_context = json.dumps(knowledge, ensure_ascii=False, indent=2) if knowledge else "{}"
...
Kết quả tham vấn nguồn tri thức dùng để định hướng, không được bịa nội dung ngoài văn bản:
<KNOWLEDGE>
{ontology_context}
</KNOWLEDGE>
...
# ---------------------------------------------------------------------------
# Long-unit LLM summarization
# ---------------------------------------------------------------------------


def split_text_by_budget(text: str, max_chars: int = 6000) -> List[str]:
    """Split long legal/article text into sentence/line-aware chunks."""
    text = (text or "").strip()
    if not text:
        return []
    ...
    # Prefer line boundaries first; if a line is too long, split by sentence.
    raw_parts: List[str] = []
    for line in text.splitlines():
        ...
        if len(line) <= max_chars:
            ...
            raw_parts.extend([normalize_space(x) for x in pieces if normalize_space(x)])

    chunks: List[str] = []
    current: List[str] = []
    current_len = 0
    for part in raw_parts:
        # Last resort for very long piece.
...
def build_chunk_summary_prompt(unit: Dict[str, Any], chunk_text: str, chunk_idx: int, chunk_count: int, max_bullets: int = 6) -> str:
    return f"""Bạn là chuyên gia tóm tắt văn bản pháp quy Việt Nam.

Đây là CHUNK {chunk_idx}/{chunk_count} của một đơn vị văn bản dài.
Hãy tóm tắt chunk này thành JSON ngắn gọn để phục vụ bước gộp sau.

Yêu cầu:
- Chỉ dùng nội dung trong chunk.
- Không thêm kiến thức ngoài.
- Không dùng dấu ba chấm.
- Không cắt câu giữa chừng.
- Trả JSON hợp lệ, không markdown.
- Tối đa {max_bullets} bullet.

Schema:
{{
  "chunk_title": "tiêu đề ngắn của chunk",
  "bullets": ["ý chính 1", "ý chính 2"],
  "key_terms": ["thuật ngữ/quy định quan trọng nếu có"]
}}
...
def build_merge_chunk_prompt(unit: Dict[str, Any], chunk_summaries: List[Dict[str, Any]], max_bullets: int = 7) -> str:
    return f"""Bạn là chuyên gia tổng hợp văn bản pháp quy Việt Nam để tạo slide.

Nhiệm vụ: Gộp các tóm tắt chunk thành MỘT slide spec cho đơn vị văn bản.

Yêu cầu:
- Chỉ dùng dữ liệu trong chunk_summaries.
- Giữ đúng ý nghĩa pháp quy; không bỏ ý quan trọng.
- Không dùng dấu ba chấm.
- Không cắt câu giữa chừng.
- bullets tối đa {max_bullets}, mỗi bullet là một ý hoàn chỉnh.
- Trả JSON object hợp lệ, không markdown.
...
def summarize_long_unit_with_gemini(
    unit: Dict[str, Any],
    client,
    model: str,
    logger: logging.Logger,
    max_task_chars: int = 7000,
    max_chunk_chars: int = 6000,
    max_bullets: int = 7,
    out_dir: str | Path | None = None,
) -> Dict[str, Any]:
    """Summarize a long unit with map-reduce LLM calls.

    If unit content <= max_task_chars, call Gemini once.
    If longer, split into chunks, summarize every chunk, then call Gemini once more to merge.
    """
    content = unit.get("content", "") or ""
    task_id = unit.get("id") or unit.get("numbering") or "unit"
    if len(content) <= max_task_chars:
        spec = summarize_unit_with_gemini(unit, client, model, logger, max_bullets=max_bullets)
        spec["long_unit_strategy"] = "single_call"
        spec["chunk_count"] = 1
        return spec

    chunks = split_text_by_budget(content, max_chars=max_chunk_chars)
    logger.info(
        "[LONG UNIT] task=%s title=%s chars=%s chunk_count=%s max_chunk_chars=%s",
        task_id,
        unit.get("title"),
        len(content),
        len(chunks),
    ...
    if out_dir:
        chunk_dir = ensure_dir(Path(out_dir) / "llm_chunk_summaries" / str(task_id).replace("/", "_"))

    for idx, chunk in enumerate(chunks, start=1):
        logger.info("[LLM CHUNK START] task=%s chunk=%s/%s chars=%s", task_id, idx, len(chunks), len(chunk))
        t0 = time.time()
        prompt = build_chunk_summary_prompt(unit, chunk, idx, len(chunks), max_bullets=6)
        raw = gemini_text_response(client, model, prompt)
        elapsed = time.time() - t0
        logger.info("[LLM CHUNK END] task=%s chunk=%s/%s elapsed=%.2fs response_chars=%s", task_id, idx, len(chunks), elapsed, len(raw or ""))
        try:
            obj = extract_json_object(raw)
        except Exception as exc:
            logger.warning("[LLM CHUNK PARSE FAIL] task=%s chunk=%s error=%s; using heuristic chunk bullets", task_id, idx, exc)
            obj = {
                "chunk_title": f"{unit.get('title')} - phần {idx}",
                "bullets": split_content_to_bullets(chunk, unit.get("kind", "article"), max_items=5),
                "key_terms": [],
                "parse_error": str(exc),
            }
        obj["chunk_index"] = idx
        obj["chunk_count"] = len(chunks)
        obj["source_chars"] = len(chunk)
        chunk_summaries.append(obj)
        if chunk_dir:
            save_json(chunk_dir / f"chunk_{idx:03d}.json", obj)
            (chunk_dir / f"chunk_{idx:03d}.txt").write_text(chunk, encoding="utf-8")

    logger.info("[LLM MERGE START] task=%s chunk_count=%s", task_id, len(chunk_summaries))
    t0 = time.time()
    merge_prompt = build_merge_chunk_prompt(unit, chunk_summaries, max_bullets=max_bullets)
    raw = gemini_text_response(client, model, merge_prompt)
    elapsed = time.time() - t0
    logger.info("[LLM MERGE END] task=%s elapsed=%.2fs response_chars=%s", task_id, elapsed, len(raw or ""))
    obj = extract_json_object(raw)
    spec = slide_spec_from_llm_result(unit, obj)
    spec["long_unit_strategy"] = "chunk_map_reduce"
    spec["chunk_count"] = len(chunks)
    spec["llm_chunk_summary_count"] = len(chunk_summaries)
    return spec


def summarize_unit_by_mode(
    unit: Dict[str, Any],
    mode: str,
    client: Any,
    model: str,
    ...
    if mode == "gemini":
        return summarize_long_unit_with_gemini(
            unit,
            client,
            model,
            logger,
    ...
    if mode == "mock":
        ...
        spec["llm_used"] = False
        spec["mock_used"] = True
        spec["long_unit_strategy"] = "mock_no_llm"
        return spec
    if mode == "heuristic":
        ...
        spec["long_unit_strategy"] = "heuristic_no_llm"
        return spec
...
def run_pipeline(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if llm_mode == "mock":
        state["mock"] = True
    logger.info("mode=legal_article_long_llm_pipeline llm_mode=%s", llm_mode)
    client = None
    if llm_mode == "gemini":
    ...
    report_ontology = load_json_file(state.get("report_ontology_path"), default={})
    presentation_ontology = load_json_file(state.get("presentation_ontology_path"), default={})
    layout_registry = load_json_file(state.get("layout_registry_path"), default={})
    logger.info(
        "[KNOWLEDGE SOURCES] report=%s loaded=%s | presentation=%s loaded=%s | registry=%s loaded=%s",
        state.get("report_ontology_path", ""), bool(report_ontology),
        state.get("presentation_ontology_path", ""), bool(presentation_ontology),
        state.get("layout_registry_path", ""), bool(layout_registry),
    )
    ...
    units = consult_knowledge_sources(units, report_ontology, presentation_ontology, layout_registry, logger)
    ...
    validation.append({
        "stage": "knowledge_consultation",
        "report_ontology_loaded": bool(report_ontology),
        "presentation_ontology_loaded": bool(presentation_ontology),
        "layout_registry_loaded": bool(layout_registry),
        "unit_decisions": [
            {
                "id": u.get("id"),
                "title": u.get("title"),
                "report_concept": u.get("report_concept"),
                "presentation_intent": u.get("presentation_intent"),
                "selected_layout": u.get("selected_layout"),
                "recommended_layout": u.get("knowledge", {}).get("layout_registry", {}).get("recommended_layout_id", u.get("selected_layout")),
            }
            for u in units
        ],
    })
    ...
    for unit in units:
        logger.info("[TASK START] unit=%s title=%s chars=%s concept=%s intent=%s layout=%s", unit.get("numbering"), unit.get("title"), len(unit.get("content", "")), unit.get("report_concept"), unit.get("presentation_intent"), unit.get("selected_layout"))
        spec = summarize_unit_by_mode(
            unit,
            llm_mode,
            client,
            state.get("model", "gemini-2.0-flash"),
            logger,
            max_task_chars=int(state.get("max_task_chars", 7000)),
            max_chunk_chars=int(state.get("max_chunk_chars", 6000)),
            out_dir=out_dir,
        )
    ...
    debug_payload = {
        ...
        "knowledge_sources": {
            "report_ontology_path": state.get("report_ontology_path", ""),
            "report_ontology_loaded": bool(report_ontology),
            "presentation_ontology_path": state.get("presentation_ontology_path", ""),
            "presentation_ontology_loaded": bool(presentation_ontology),
            "layout_registry_path": state.get("layout_registry_path", ""),
            "layout_registry_loaded": bool(layout_registry),
        },
        "units": units,
        "tasks": units,
        ...
        "llm": {
            ...
            "task_llm_call_count": sum((1 + max(0, int(sl.get("chunk_count", 1)) - 1) + (1 if sl.get("long_unit_strategy") == "chunk_map_reduce" else 0)) for sl in slides) if llm_mode == "gemini" else 0,
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="out_legal_article_long_llm_ontology")
