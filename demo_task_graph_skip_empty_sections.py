#!/usr/bin/env python3
"""
Task-graph ontology-guided PDF/DOCX/TXT/PLAIN-TEXT -> slide deck pipeline with empty-section skipping
======================================================
...
"""
    ...
    if skipped:
        state.setdefault("validation_reports", []).append(
            {"stage": "filter_empty_report_sections", "skipped_sections": skipped}
        )
        for item in skipped:
            state.setdefault("repair_log", []).append(
                f"Skipped report component {item.get('concept_id')}: {item.get('reason')}"
            )
    return state
...
def normalize_summary(summary: Dict[str, Any], ontology: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize LLM summary and DROP ontology components with no extracted content.

    Previous versions inserted fallback bullets such as "Chưa trích xuất được...".
    That created empty-looking slides. This version keeps provenance honest:
    if a report component has no meaningful bullets/conclusion, it is skipped
    and recorded in `skipped_sections`.
    """
    ...
    for concept in ontology.get("concepts", []):
        ...
        max_bullets = int(concept.get("max_bullets", 5))
        ...
        if not bullets and not has_conclusion:
            skipped_sections.append(
                {
                    "concept_id": cid,
                    "heading": concept.get("preferred_heading", cid),
    ...
    return {
        "title": safe_text(summary.get("title"), "Báo cáo tổng hợp"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Task-graph ontology slide generation pipeline with empty-section skipping")
