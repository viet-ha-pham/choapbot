...
- nested bullets: multi-level bullet trees with icons per level and per item
- inline Markdown styles in text: **bold**, *italic*, __bold__, _italic_, ~~strike~~, `code`
...
# Keys that are normally used as mapping keys in generic PML/PSL/PCL blocks.
# If a line has a colon but the part before ':' is not one of these keys or does
# not look like a technical identifier, it is treated as plain text.
KNOWN_MAPPING_KEYS = {
    "align", "align_side", "alt", "author", "auto_shrink", "background", "background_image",
    "base_width", "border", "border_color", "bullet_icon", "bullets", "caption", "cell_fill",
    "cells", "color", "columns", "date", "fill", "font", "format", "gap", "header_color",
    "header_fill", "heading", "height", "icon", "image", "items", "language", "layout",
    "line_gap", "link", "max_items_per_slide", "max_lines", "max_rows_per_slide", "min_size",
    "mode", "opacity", "overflow", "padding", "position", "radius", "rows", "shrink", "size",
    "src", "steps", "subtitle", "table", "text", "title", "underline", "url", "width",
    "x", "y", "w", "h", "bold", "italic", "headers", "footer", "footer_text",
    "footer_image", "footer_images", "left", "right", "top", "bottom", "card", "grid",
}

TEXT_BLOCK_KEYS = {"title", "subtitle", "caption", "notes", "footer_text", "text", "body", "description"}


def looks_like_mapping_text(text: str) -> bool:
    """Return True only when a colon line looks like DSL mapping syntax.

    This prevents plain text such as `Problem: data overload` or
    `Lưu ý: cần kiểm tra dữ liệu` from being parsed as a key/value pair.
    """
    if ":" not in text:
        return False
    ...
    if not key:
        return False
    if key in KNOWN_MAPPING_KEYS:
        return True
    # Technical keys used by styles often look like identifiers: header_fill,
    # background-image, slide.title, etc. Natural language usually contains spaces,
    # accents, or punctuation beyond this set.
    return bool(re.match(r"^[A-Za-z_][A-Za-z0-9_.-]*$", key))


def split_key_value_safe(text: str) -> Tuple[str, Optional[str]]:
    """Split `key: value` only if the line is likely to be DSL mapping syntax."""
    if not looks_like_mapping_text(text):
        return text, None
    return split_key_value(text)


def parse_text_block(lines: List[Line], start: int, indent: int) -> Tuple[str, int]:
    """Parse an indented multiline scalar exactly as text, preserving colons."""
    text_lines: List[str] = []
    i = start
    while i < len(lines):
        line = lines[i]
        if line.indent < indent:
            break
        # Preserve relative indentation inside text blocks.
        if line.indent >= indent:
            extra = " " * max(0, line.indent - indent)
            text_lines.append(extra + line.text)
            i += 1
        else:
            break
    return "\n".join(text_lines), i
    ...
    while i < len(lines):
        ...
        if text.startswith("- "):
            ...
            if item_text.endswith(":"):
                ...
                i = new_i
            elif ":" in item_text and item_text.split(":", 1)[0].strip() in KNOWN_MAPPING_KEYS:
        ...
        key, val = split_key_value_safe(text)
        if val is None:
            text_lines = []
    ...
    while i < len(lines):
        ...
        if val == "":
            if i + 1 >= len(lines) or lines[i + 1].indent <= line.indent:
                ...
                child, new_i = parse_text_block(lines, i + 1, lines[i + 1].indent)
...
def derive_subtitle_style(layout_style: Dict[str, Any], theme: Theme) -> Dict[str, Any]:
    """Return a safe subtitle style.

    Many layouts define `title` but not `subtitle`. The old fallback rendered
    subtitle with an empty/default style or reused title style, which could put
    subtitle on top of the title. This function derives a reasonable subtitle
    position directly below the title when no explicit subtitle style exists.
    """
    if not isinstance(layout_style, dict):
        return {}
    if isinstance(layout_style.get("subtitle"), dict) and layout_style.get("subtitle"):
        return layout_style["subtitle"]

    title_style = layout_style.get("title", {}) if isinstance(layout_style.get("title", {}), dict) else {}
    pos = title_style.get("position", [60, 40])
    title_h = safe_float(title_style.get("height", 70), 70)
    title_size = safe_float(title_style.get("size", 34), 34)

    # Keep subtitle visually attached to title, but clearly below it.
    return {
        "position": [pos[0], pos[1] + title_h + 6],
        "width": title_style.get("width", 1000),
        "height": 44,
        "font": title_style.get("font", theme.fonts.get("body", "Aptos")),
        "size": max(16, int(title_size * 0.58)),
        "color": theme.colors.get("muted", title_style.get("color", "#666666")),
        "align": title_style.get("align", "left"),
    }


def get_text_style_for_key(layout_style: Dict[str, Any], key: str, theme: Theme) -> Dict[str, Any]:
    """Get style for a text block with safe defaults for common keys.

    Critical fix: `subtitle` must not default to title/default position because
    that causes overlap. If a layout lacks subtitle style, derive it below title.
    """
    if key == "subtitle":
        return derive_subtitle_style(layout_style, theme)
    style = layout_style.get(key, {}) if isinstance(layout_style, dict) else {}
    return style if isinstance(style, dict) else {}

def normalize_text_style_aliases(kind: str, style: Dict[str, Any], theme: Theme) -> Dict[str, Any]:
    """Normalize common text style aliases before creating TextBox.

    This makes layouts more forgiving. In generated PSL/registry-driven layouts,
    title styles sometimes arrive as `heading_font`/`heading_color` or
    `title_font`/`title_color` instead of the canonical `font`/`color`.
    Without this normalization, title-bullets can silently fall back to Aptos /
    black even though the layout intended to use heading font and primary color.
    """
    st = dict(style or {})
    role = str(kind or "").lower()

    if "font" not in st:
        for alias in ("title_font", "heading_font", "label_font") if role in {"title", "slide.title"} else ("body_font", "text_font"):
            if alias in st:
                st["font"] = st[alias]
                break
    if "color" not in st:
        for alias in ("title_color", "heading_color", "label_color") if role in {"title", "slide.title"} else ("body_color", "text_color"):
            if alias in st:
                st["color"] = st[alias]
                break

    # Strong defaults by semantic role. This prevents a missing title style from
    # looking like body text.
    if role in {"title", "slide.title"}:
        st.setdefault("font", "heading")
        st.setdefault("size", 34)
        st.setdefault("color", "primary")
        st.setdefault("position", [60, 40])
        st.setdefault("width", 1100)
        st.setdefault("height", 80)
    elif role == "subtitle":
        st.setdefault("font", "body")
        st.setdefault("color", "muted")
    else:
        st.setdefault("font", "body")
        st.setdefault("color", "text")

    return st


def make_text_object(kind: str, value: Any, style: Dict[str, Any], theme: Theme) -> Dict[str, Any]:
    style = normalize_text_style_aliases(kind, style, theme)
...
def normalize_conclusion_block(value: Any) -> Dict[str, Any]:
    """Normalize a highlighted conclusion/summary box block.

    Supported PML:
      conclusion:
        Cần xây dựng hệ thống tự động hóa.

      conclusion:
        icon: check
        text: Cần xây dựng hệ thống tự động hóa.
        fill: "#E0F2FE"
        border: primary
        align: center
        url: "https://example.com"
      ...
      summary_box:
        text: Một kết luận nổi bật ở cuối slide.
    """
    if isinstance(value, dict):
        text = value.get("text", value.get("summary", value.get("content", value.get("body", ""))))
        if text == "" and "heading" in value:
            text = value.get("heading", "")
        result = dict(value)
        result["text"] = str(text)
        return result
    return {"text": normalize_text_block(value)}


def make_conclusion_objects(value: Any, style: Dict[str, Any], theme: Theme) -> List[Dict[str, Any]]:
    """Build a highlighted conclusion textbox near the bottom of a slide.

    The box is rendered as a Card shape plus a TextBox placed on top, so it works
    consistently in both HTML and PPTX. PML values override PSL style values for
    content-level customization such as fill/border/icon.
    """
    cfg = normalize_conclusion_block(value)
    merged_style = dict(style or {})
    for key in (
        "position", "width", "height", "fill", "background", "border", "border_color",
        "radius", "padding", "font", "size", "color", "bold", "italic", "underline",
        "align", "overflow", "max_lines", "min_size", "auto_shrink", "url", "href", "link",
    ):
        if key in cfg:
            merged_style[key] = cfg[key]

    pos = merged_style.get("position", [80, 555])
    x = safe_float(pos[0] if isinstance(pos, list) and len(pos) > 0 else 80, 80)
    y = safe_float(pos[1] if isinstance(pos, list) and len(pos) > 1 else 555, 555)
    w = safe_float(merged_style.get("width", 1120), 1120)
    h = safe_float(merged_style.get("height", 88), 88)
    padding = safe_float(merged_style.get("padding", 18), 18)

    fill_value = cfg.get("fill", cfg.get("background", merged_style.get("fill", merged_style.get("background", theme.colors.get("secondary", "#E0F2FE")))))
    border_value = cfg.get("border", cfg.get("border_color", merged_style.get("border", merged_style.get("border_color", theme.colors.get("primary", "#2563EB")))))

    card = {
        "type": "Card",
        "role": "conclusion.card",
        "x": x,
        "y": y,
        "w": w,
        "h": h,
        "fill": resolve_token(fill_value, theme),
        "border": resolve_token(border_value, theme),
        "radius": merged_style.get("radius", 18),
    }

    icon = resolve_bullet_icon(cfg.get("icon", cfg.get("bullet_icon", merged_style.get("icon", ""))), "")
    text = str(cfg.get("text", ""))
    if icon:
        text = f"{icon} {text}".strip()

    text_style = dict(merged_style)
    text_style.setdefault("position", [x + padding, y + padding])
    text_style.setdefault("width", max(10, w - 2 * padding))
    text_style.setdefault("height", max(10, h - 2 * padding))
    text_style.setdefault("font", theme.fonts.get("body", "Aptos"))
    text_style.setdefault("size", 22)
    text_style.setdefault("color", theme.colors.get("text", "#111827"))
    text_style.setdefault("bold", True)
    text_style.setdefault("align", "center")
    text_style.setdefault("overflow", "shrink")
    text_style.setdefault("min_size", 14)

    textbox = make_text_object("conclusion", {"text": text, "url": get_url(cfg, merged_style)}, text_style, theme)
    return [card, textbox]
...
def normalize_level_icons(value: Any, theme: Optional[Theme] = None) -> Dict[int, str]:
    """Normalize level icon config.

    Supported forms:
      level_icons: [diamond, arrow, dot]
      level_icons:
        level0: diamond
        level1: arrow
        2: dot
    ...
    if isinstance(value, list):
        for idx, icon in enumerate(value):
            icons[idx] = resolve_bullet_icon(icon, "")
    elif isinstance(value, dict):
        for key, icon in value.items():
            key_text = str(key).strip().lower().replace("level", "")
            try:
                level = int(key_text)
            except ValueError:
                continue
            icons[level] = resolve_bullet_icon(icon, "")
    return icons


def bullet_default_icon_for_level(level: int, default_icon: str, level_icons: Dict[int, str]) -> str:
    if level in level_icons:
        return level_icons[level]
    if level == 0:
        return default_icon
    # Sensible fallback for nested children when not configured explicitly.
    fallback = {1: "→", 2: "●", 3: "–"}
    return fallback.get(level, "•")


def normalize_bullet_items(
    value: Any,
    default_icon: str = "",
    level_icons: Optional[Dict[int, str]] = None,
    level: int = 0,
) -> List[Dict[str, Any]]:
    """Normalize bullets to a recursive tree of items.
    ...
    Supported PML:
      ...
      bullets:
        ...
        - icon: warning
          text: B
    """
    level_icons = level_icons or {}

    if isinstance(value, dict) and ("items" in value or "bullets" in value):
        # Root/list config block.
        default_icon = resolve_bullet_icon(value.get("icon", value.get("bullet_icon")), default_icon)
        local_level_icons = normalize_level_icons(value.get("level_icons", value.get("icons_by_level", {})))
        level_icons = {**level_icons, **local_level_icons}
    ...
    for raw in raw_items:
        if isinstance(raw, dict):
            inherited_icon = bullet_default_icon_for_level(level, default_icon, level_icons)
            icon = resolve_bullet_icon(raw.get("icon", raw.get("bullet_icon")), inherited_icon)
            text = raw.get("text", raw.get("label", raw.get("title", raw.get("heading", ""))))

            # Handle parser output like {'Some label': {...}} as a compact parent form.
            compact_children: Any = None
            if text == "" and len(raw) == 1:
                only_key, only_val = next(iter(raw.items()))
                text = only_key
                compact_children = only_val

            child_raw = raw.get("children", raw.get("items", raw.get("bullets", compact_children if compact_children is not None else [])))
            children = normalize_bullet_items(
                child_raw,
                default_icon=default_icon,
                level_icons=level_icons,
                level=level + 1,
            ) if child_raw not in (None, "", []) else []
            ...
            items.append({
                "icon": bullet_default_icon_for_level(level, default_icon, level_icons),
                "text": str(raw),
                "url": "",
                "level": level,
                "children": [],
...
def flatten_bullet_items(items: List[Dict[str, Any]], level: int = 0) -> List[Tuple[Dict[str, Any], int]]:
    flat: List[Tuple[Dict[str, Any], int]] = []
    for item in items or []:
        if isinstance(item, dict):
            item_level = safe_int(item.get("level", level), level)
            flat.append((item, item_level))
            flat.extend(flatten_bullet_items(item.get("children", []), item_level + 1))
        else:
            flat.append(({"icon": "", "text": str(item), "url": "", "children": []}, level))
    return flat
...
def make_bullet_object(value: Any, style: Dict[str, Any], theme: Theme) -> Dict[str, Any]:
    ...
    for key in (
        ...
        "level_icons", "icons_by_level", "level_indent",
    ):
    ...
    level_icons = normalize_level_icons(effective_style.get("level_icons", effective_style.get("icons_by_level", {})))
    icon_color = style_value(effective_style, "icon_color", theme, style_value(effective_style, "color", theme, "#000000"))
    return {
        "type": "BulletList",
        "items": normalize_bullet_items(value, default_icon, level_icons),
        "x": pos[0],
        "y": pos[1],
        "w": style.get("width", 1000),
        ...
        "level_indent": effective_style.get("level_indent", 28),
        "level_icons": level_icons,
...
def build_text_objects_from_block(
    ...
    for key in keys:
        if key not in block:
            continue
        style = get_text_style_for_key(layout_style, key, theme)
...
def split_bullet_items_by_height(obj: Dict[str, Any]) -> List[List[Dict[str, Any]]]:
    ...
    """
    # Flatten nested bullets for pagination. This intentionally keeps parent before children.
    items = [dict(item, level=level) for item, level in flatten_bullet_items(obj.get("items", []) or [])]
    if not items:
        return [[]]
    ...
    def item_height(item: Dict[str, Any]) -> float:
        ...
        level = safe_int(item.get("level", 0), 0) if isinstance(item, dict) else 0
        nested_size = max(safe_float(obj.get("min_size", 12), 12), size - level * 1.5)
        nested_w = max(20.0, text_w - level * safe_float(obj.get("level_indent", 28), 28))
        return estimate_text_height(text, nested_w, nested_size, line_gap) + max(4.0, line_gap)
        ...
        for slide in section.slides:
            ...
            if slide.layout == "image-caption":
                ...
                if "subtitle" in slide.blocks:
                    objects.append(make_text_object("subtitle", slide.blocks["subtitle"], get_text_style_for_key(layout_style, "subtitle", theme), theme))
            ...
            # Highlighted conclusion/summary box, normally placed after bullets and before footer.
            if "conclusion" in slide.blocks or "summary_box" in slide.blocks:
                conclusion_key = "conclusion" if "conclusion" in slide.blocks else "summary_box"
                conclusion_style = layout_style.get(conclusion_key, layout_style.get("conclusion", {}))
                objects.extend(make_conclusion_objects(slide.blocks[conclusion_key], conclusion_style, theme))
...
def apply_pptx_text_style(paragraph, obj: Dict[str, Any]) -> None:
    """Apply style to both paragraph defaults and existing runs.

    python-pptx may create a run when `p.text = ...` is assigned. Setting only
    `paragraph.font` is usually enough for defaults, but some viewers/templates
    prioritize run properties. Styling the runs explicitly makes title font/color
    reliable, especially for `title-bullets`.
    """
    paragraph.alignment = pptx_alignment(obj.get("align", "left"))
    font_name = obj.get("font", "Aptos")
    font_size = Pt(obj.get("_render_size", obj.get("size", 20)))
    font_color = hex_to_rgb(obj.get("color", "#000000"))
    bold = parse_bool(obj.get("bold", False))
    italic = parse_bool(obj.get("italic", False))
    underline = parse_bool(obj.get("underline", False))
...
def add_textbox(slide, obj: Dict[str, Any], bullet: bool = False) -> None:
    ...
    if bullet:
        flat_items = flatten_bullet_items(obj.get("items", []))
        for idx, (item, level) in enumerate(flat_items):
            p = tf.paragraphs[0] if idx == 0 else tf.add_paragraph()
            raw_text = bullet_item_to_plain_text(item)
            text, render_size = prepare_pptx_text(raw_text, obj)
            p.level = max(0, min(8, safe_int(level, 0)))
            obj2 = dict(obj)
            # Slightly reduce nested bullet font size for hierarchy clarity.
            obj2["_render_size"] = max(safe_float(obj.get("min_size", 12), 12), render_size - max(0, level) * 1.5)
            set_paragraph_inline_markdown(p, text, obj2, str(item.get("url", "")) if isinstance(item, dict) else "")
    else:
        p = tf.paragraphs[0]
        text, render_size = prepare_pptx_text(obj["text"], obj)
        obj2 = dict(obj)
        obj2["_render_size"] = render_size
        set_paragraph_inline_markdown(p, text, obj2, str(obj.get("url", "")))
...
def add_table(slide, obj: Dict[str, Any]) -> None:
    ...
    def set_cell(cell, text: str, *, is_header: bool = False, row_idx: int = 0) -> None:
        ...
        for paragraph in cell.text_frame.paragraphs:
            ...
            cell_obj["bold"] = bold
            set_paragraph_inline_markdown(paragraph, text, cell_obj)
...
# -----------------------------------------------------------------------------
# Inline Markdown style parser
# -----------------------------------------------------------------------------

def parse_inline_markdown(text: Any) -> List[Dict[str, Any]]:
    """Parse a small, safe subset of inline Markdown into styled text runs.

    Supported markers:
      **bold**, __bold__, *italic*, _italic_, ***bold italic***,
      ___bold italic___, ~~strike~~, `code`.

    The parser is intentionally conservative: unmatched markers are preserved as
    literal text so generated PML never breaks rendering. Links are handled by
    object-level `url` fields rather than Markdown link syntax.
    """
    src = "" if text is None else str(text)
    runs: List[Dict[str, Any]] = []
    buf: List[str] = []
    state = {"bold": False, "italic": False, "strike": False, "code": False}

    def flush() -> None:
        if buf:
            runs.append({"text": "".join(buf), **state})
            buf.clear()

    i = 0
    while i < len(src):
        # Escape next character, e.g. \* keeps literal *.
        if src[i] == "\\" and i + 1 < len(src):
            buf.append(src[i + 1])
            i += 2
            continue

        # Inline code disables other marker parsing until next backtick.
        if src[i] == "`":
            flush()
        ...
        if src.startswith("***", i) or src.startswith("___", i):
            flush()
            state["bold"] = not state["bold"]
            state["italic"] = not state["italic"]
            i += 3
            continue
        if src.startswith("**", i) or src.startswith("__", i):
            flush()
            state["bold"] = not state["bold"]
            i += 2
            continue
...
def html_inline_markdown(text: Any, url: str = "") -> str:
    """Render inline Markdown subset to safe HTML spans/a."""
    parts: List[str] = []
    for run in parse_inline_markdown(text):
        escaped = html.escape(str(run.get("text", "")))
        if not escaped:
            continue
        styles: List[str] = []
        if run.get("bold"):
        ...
        if run.get("code"):
            styles.extend(["font-family:ui-monospace, SFMono-Regular, Consolas, monospace", "background:rgba(148,163,184,.18)", "padding:0 3px", "border-radius:4px"])
        style_attr = f" style='{'; '.join(styles)}'" if styles else ""
        parts.append(f"<span{style_attr}>{escaped}</span>")
    inner = "".join(parts)
    if url:
        return f"<a{html_link_attrs(url)}>{inner}</a>"
    return inner


def apply_pptx_run_style(run, obj: Dict[str, Any], extra: Optional[Dict[str, Any]] = None) -> None:
    """Apply base object style plus inline Markdown style flags to one PPTX run."""
    extra = extra or {}
    run.font.name = obj.get("font", "Aptos")
    run.font.size = Pt(obj.get("_render_size", obj.get("size", 20)))
    run.font.color.rgb = hex_to_rgb(obj.get("color", "#000000"))
    run.font.bold = parse_bool(obj.get("bold", False)) or parse_bool(extra.get("bold", False))
    run.font.italic = parse_bool(obj.get("italic", False)) or parse_bool(extra.get("italic", False))
    # PPTX does not have a portable strikethrough API in python-pptx.
    run.font.underline = parse_bool(obj.get("underline", False))
    if extra.get("code"):
        run.font.name = "Consolas"


def set_paragraph_inline_markdown(paragraph, text: Any, obj: Dict[str, Any], url: str = "") -> None:
    """Set paragraph content using inline Markdown runs."""
    paragraph.clear()
    paragraph.alignment = pptx_alignment(obj.get("align", "left"))

    # Default paragraph font, useful when the paragraph starts empty.
    paragraph.font.name = obj.get("font", "Aptos")
    paragraph.font.size = Pt(obj.get("_render_size", obj.get("size", 20)))
    paragraph.font.color.rgb = hex_to_rgb(obj.get("color", "#000000"))
    ...
    for run_spec in parse_inline_markdown(text):
        run_text = str(run_spec.get("text", ""))
        if run_text == "":
            continue
        run = paragraph.add_run()
        run.text = run_text
        apply_pptx_run_style(run, obj, run_spec)
        if url:
            try:
...
# -----------------------------------------------------------------------------
# Markdown renderer for LLM evaluation / G-Eval
# -----------------------------------------------------------------------------

def md_escape(value: Any) -> str:
    """Keep Markdown readable while avoiding accidental table/list breakage."""
...
def md_role_label(role: Any) -> str:
    role_text = str(role or "content").replace("_", " ").replace(".", " / ").strip()
    return role_text[:1].upper() + role_text[1:]


def md_format_link(text: Any, url: Any) -> str:
    text_s = md_inline(text)
    url_s = md_inline(url)
    if url_s:
        return f"{text_s} ({url_s})"
    return text_s


def md_bullet_lines(items: Any, level: int = 0) -> List[str]:
    """Render normal/icon/nested bullets as plain semantic Markdown.

    Some renderer stages flatten nested bullets into items with an explicit
    ``level`` field. In that case, render the flat sequence exactly once and do
    not recurse into ``children``; otherwise child bullets would be duplicated in
    the Markdown and LLM judges may penalize the summary unfairly.
    """
    lines: List[str] = []
    if not isinstance(items, list):
        items = [items]

    has_explicit_levels = any(isinstance(item, dict) and "level" in item for item in items)
    for item in items:
        if isinstance(item, dict):
            item_level = safe_int(item.get("level", level), level) if has_explicit_levels else level
            indent = "  " * max(0, item_level)
            text = md_format_link(item.get("text", ""), item.get("url", ""))
            icon = md_inline(item.get("icon", ""))
            prefix = f"[{icon}] " if icon else ""
            if text:
                lines.append(f"{indent}- {prefix}{text}")
            if not has_explicit_levels:
                children = item.get("children", []) or []
                if children:
                    lines.extend(md_bullet_lines(children, level + 1))
        else:
            indent = "  " * max(0, level)
            lines.append(f"{indent}- {md_inline(item)}")
    return lines


def md_table(headers: Any, rows: Any) -> List[str]:
    headers = headers or []
    rows = rows or []
    lines: List[str] = []
    if not headers and rows:
        max_cols = max(len(r) if isinstance(r, list) else 1 for r in rows)
        headers = [f"Column {i+1}" for i in range(max_cols)]
    if not headers:
        return lines
    safe_headers = [md_inline(h).replace("|", "\\|") for h in headers]
    lines.append("| " + " | ".join(safe_headers) + " |")
    lines.append("| " + " | ".join(["---"] * len(safe_headers)) + " |")
    for row in rows:
        if not isinstance(row, list):
            row = [row]
        cells = [md_inline(c).replace("|", "\\|") for c in row]
...
def object_to_markdown(obj: Dict[str, Any]) -> List[str]:
    obj_type = obj.get("type")
    role = obj.get("role", obj_type)
    lines: List[str] = []

    if obj_type == "TextBox":
        text = md_escape(obj.get("text", ""))
        if not text:
            return []
        label = md_role_label(role)
        if str(role).lower() in {"title", "slide title"}:
            lines.append(f"### {md_inline(text)}")
        elif str(role).lower() in {"subtitle", "caption", "footer", "footer_text", "conclusion", "summary", "body", "link"}:
            lines.append(f"**{label}:** {md_format_link(text, obj.get('url', ''))}")
        else:
            lines.append(f"**{label}:** {md_format_link(text, obj.get('url', ''))}")
        return lines

    if obj_type == "BulletList":
        role_label = "Key points" if str(role or "").lower() in {"bulletlist", "bullets"} else md_role_label(role if role else "Key points")
        lines.append(f"**{role_label}:**")
        lines.extend(md_bullet_lines(obj.get("items", []), 0))
        return lines

    if obj_type == "TableBox":
        lines.append("**Table:**")
        table_lines = md_table(obj.get("headers", []), obj.get("rows", []))
        if table_lines:
            lines.extend(table_lines)
        return lines

    if obj_type == "Image":
        alt = md_inline(obj.get("alt", ""))
        src = md_inline(obj.get("src", ""))
        role_label = md_role_label(role if role else "Image")
        if alt and src:
            lines.append(f"**{role_label}:** {alt} [image: {src}]")
        elif src:
            lines.append(f"**{role_label}:** [image: {src}]")
        return lines

    if obj_type == "Card":
        # Cards are visual containers; content text usually appears as sibling
        # TextBox/BulletList objects. Record them only as structure hints.
        role_label = md_role_label(role if role else "Card")
        lines.append(f"<!-- Visual container: {role_label} -->")
        return lines

    if obj_type in {"Circle", "LineBox"}:
        # Decorative timeline/step elements; omit to avoid noisy evaluation text.
        return []

    # Conservative fallback for future objects.
    text = md_escape(obj.get("text", ""))
    if text:
        lines.append(f"**{md_role_label(role)}:** {text}")
    return lines


def render_markdown(render_ir: Dict[str, Any], output_path: str) -> None:
    """Render deck to an evaluation-friendly Markdown representation.

    Design goals for G-Eval/LLM judges:
    - preserve slide boundaries and order;
    - expose section/layout metadata without overwhelming the evaluator;
    - linearize visual elements into semantic text;
    - preserve nested bullet hierarchy, tables, captions, links, notes;
    - avoid decorative objects that could lower scores by adding noise.
    """
    parts: List[str] = []
    title = md_inline(render_ir.get("title", "Untitled Presentation"))
    slides = render_ir.get("slides", []) or []

    parts.append(f"# {title}")
    parts.append("")
    parts.append("<!-- Markdown rendering generated for LLM-based evaluation. -->")
    parts.append(f"<!-- Slide count: {len(slides)} -->")
    parts.append("")
    ...
    for idx, slide in enumerate(slides, start=1):
        ...
        if section and section != current_section:
            ...
            parts.append(f"## Section: {md_inline(section)}")
            parts.append("")

        slide_title = md_inline(slide.get("slide_title", f"Slide {idx}"))
        kind = md_inline(slide.get("kind", "slide"))
        layout = md_inline(slide.get("layout", ""))

        parts.append(f"## Slide {idx}: {slide_title}")
        meta = []
        if kind:
            meta.append(f"kind={kind}")
        if layout:
            meta.append(f"layout={layout}")
        if section:
            meta.append(f"section={md_inline(section)}")
        if meta:
        ...
        for obj in slide.get("objects", []) or []:
            object_lines.extend(object_to_markdown(obj))
            if object_lines and object_lines[-1] != "":
                object_lines.append("")
        # remove visual-container-only comments if slide has other semantic content
        semantic = [line for line in object_lines if line and not line.startswith("<!-- Visual container")]
        if semantic:
            object_lines = [line for line in object_lines if not line.startswith("<!-- Visual container")]
        parts.extend(object_lines or ["_(No textual content)_", ""])

        notes = md_escape(slide.get("notes", ""))
        if notes:
    ...
    for slide in render_ir["slides"]:
        ...
        for obj in slide["objects"]:
            if obj["type"] == "Card":
                ...
                url = str(obj.get("url", ""))
                parts.append(f"<div class='textbox' style='{style}'>" + html_inline_markdown(obj["text"], url) + "</div>")
            elif obj["type"] == "BulletList":
                style = (
                ...
                icon_gap = safe_float(obj.get("icon_gap", 12), 12)
                line_gap = safe_float(obj.get("line_gap", 8), 8)
                icon_size = safe_float(obj.get("icon_size", obj.get("size", 24)), 24)
                level_indent = safe_float(obj.get("level_indent", 28), 28)
                icon_color = html.escape(str(obj.get("icon_color", obj.get("color", "#000000"))))

                def render_bullet_item(item: Any, level: int = 0) -> str:
                    if isinstance(item, dict):
                        ...
                        url = ""
                        children = []
                    text_html = html_inline_markdown(text_raw, url)
                    nested_font_size = max(safe_float(obj.get("min_size", 12), 12), safe_float(obj.get("size", 24), 24) - level * 1.5)
                    margin_left = level * level_indent
                    if icon:
                        row = (
                            f"<div style='display:flex; align-items:flex-start; gap:{icon_gap}px; margin-bottom:{line_gap}px; margin-left:{margin_left}px; font-size:{nested_font_size}px;'>"
                            f"<span style='color:{icon_color}; font-size:{max(10, icon_size - level * 1.5)}px; line-height:1.2; min-width:{icon_size}px; flex:0 0 {icon_size}px;'>"
                            f"{icon}</span><span style='line-height:1.25; flex:1; min-width:0; overflow-wrap:anywhere;'>{text_html}</span></div>"
                        )
                    else:
                        row = f"<div style='margin-bottom:{line_gap}px; margin-left:{margin_left}px; line-height:1.25; font-size:{nested_font_size}px;'>• {text_html}</div>"
                    child_html = "".join(render_bullet_item(child, level + 1) for child in children)
                    return row + child_html

                parts.append(f"<div class='bullets' style='{style}'>")
                for item in obj.get("items", []):
                    parts.append(render_bullet_item(item, 0))
                parts.append("</div>")
            elif obj["type"] == "TableBox":
                style = (
                ...
                if obj.get("headers"):
                    ...
                    for cell in obj.get("headers", []):
                        parts.append(
                            ...
                            f"{html_inline_markdown(cell)}</th>"
                        )
                ...
                for idx, row in enumerate(obj.get("rows", [])):
                    ...
                    for cell in row:
                        parts.append(
                            ...
                            f"{html_inline_markdown(cell)}</td>"
                        )
                    parts.append("</tr>")
  ...
  section "Bối cảnh":
    ...
    slide "Vấn đề hiện tại":
      ...
      bullets:
        ...
        level_icons: [diamond, arrow, dot]
        items:
          - text: Dữ liệu phân tán ở nhiều hệ thống
            children:
              - icon: database
                text: CRM, ERP, báo cáo nội bộ
              - icon: link
                text: Nguồn dữ liệu chưa được liên kết
          - text: Báo cáo thủ công mất nhiều thời gian
            children:
              - text: Thu thập thủ công
              - text: Tổng hợp thủ công
          - icon: warning
            text: Lãnh đạo cần insight nhanh hơn, nhưng nội dung bullet đôi khi rất dài nên cần cơ chế tự xuống dòng, co chữ hoặc cắt gọn để không phá bố cục slide.

      conclusion:
        icon: check
        text: Cần chuyển từ báo cáo thủ công sang hệ thống tổng hợp tri thức tự động, có kiểm soát và có khả năng sinh slide.
...
def main() -> None:
    ...
    render_markdown(render_ir, "demo_output.md")
    ...
    print("Generated demo_output.md")
