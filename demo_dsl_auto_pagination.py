...
def make_table_object(value: Any, style: Dict[str, Any], theme: Theme) -> Dict[str, Any]:
    ...
    return {
        ...
        "overflow": normalize_overflow(style.get("overflow", "wrap")),
        "max_rows_per_slide": safe_int(style.get("max_rows_per_slide", style.get("rows_per_slide", 0)), 0),
    }
...
def make_bullet_object(value: Any, style: Dict[str, Any], theme: Theme) -> Dict[str, Any]:
    ...
    default_icon = resolve_bullet_icon(style.get("icon", style.get("bullet_icon")), "")
    icon_color = style_value(style, "icon_color", theme, style_value(style, "color", theme, "#000000"))
    return {
        "type": "BulletList",
        ...
        "icon_size": style.get("icon_size", style.get("size", 24)),
        "icon_gap": style.get("icon_gap", 12),
        "max_items_per_slide": safe_int(
            (value.get("max_items_per_slide") if isinstance(value, dict) else None)
            or style.get("max_items_per_slide")
            or style.get("items_per_slide"),
            0,
        ),
        **text_style_props(style),
    }
...
def paginate_single_slide(slide_ir: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Split one render slide into multiple slides when a main object asks for pagination.

    Supported objects:
    - BulletList with overflow: paginate and max_items_per_slide/items_per_slide
    - TableBox with overflow: paginate and max_rows_per_slide/rows_per_slide

    The function intentionally paginates one dominant object per slide. This keeps the
    rule predictable for presentation authoring. Other objects such as title, footer,
    background, and decorative shapes are repeated on every continuation slide.
    """
    ...
    for idx, obj in enumerate(objects):
        if obj.get("type") == "BulletList" and obj.get("overflow") == "paginate":
            items = obj.get("items", [])
            per_page = safe_int(obj.get("max_items_per_slide", 0), 0)
            if per_page <= 0:
                # Practical default based on a normal title-bullets slide.
                per_page = max(4, int(safe_float(obj.get("h", 420), 420) // 52))
            if len(items) <= per_page:
                return [slide_ir]
            pages = []
            for page_index, chunk in enumerate(chunk_list(items, per_page)):
        ...
        if obj.get("type") == "TableBox" and obj.get("overflow") == "paginate":
            rows = obj.get("rows", [])
            per_page = safe_int(obj.get("max_rows_per_slide", 0), 0)
            if per_page <= 0:
                per_page = max(3, int(safe_float(obj.get("h", 430), 430) // 42) - 1)
            if len(rows) <= per_page:
                return [slide_ir]
            pages = []
            for page_index, chunk in enumerate(chunk_list(rows, per_page)):
                new_slide = dict(slide_ir)
                new_objects = [clone_obj(o) for o in objects]
