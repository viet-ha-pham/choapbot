...
4. Chỉ tạo nhiều section khi văn bản phân cấp đủ sâu và mỗi section dự kiến có ít nhất 2 slide nội dung.
5. Không tạo section vụn vặt chỉ có 1 slide; nếu một nhóm chỉ có 1 slide thì gộp vào section lân cận hoặc group chung "Nội dung chính".
6. Ontology chỉ là gợi ý nhãn nghiệp vụ, không được thay thế đề mục nguồn.
7. Bỏ qua quốc hiệu, tiêu ngữ, số văn bản, nơi nhận, ký tá.
8. Chỉ tạo section boundary khi nó giúp deck dễ đọc: phần lớn, chương, nhóm đề mục nhiều slide, hoặc chuyển chủ đề rõ.
9. Nếu một heading là mục con nhỏ, đặt parent_id rõ ràng và role = "supporting_detail" hoặc "slide" tùy độ quan trọng.
...
# -----------------------------------------------------------------------------
# Section grouping policy
# -----------------------------------------------------------------------------

MIN_SLIDES_PER_SECTION = 2
MIN_TOTAL_SLIDES_FOR_MULTIPLE_SECTIONS = 4
MIN_GROUPS_FOR_SECTION_HEADERS = 2


def _outline_has_real_depth(hierarchy: Dict[str, Any]) -> bool:
    """Return True only when the detected document outline has a meaningful hierarchy.

    We intentionally do not trust every LLM-proposed group. A deck should only get
    multiple visual sections when the source has clear depth: level-1/level-2
    structure, parent-child relations, or enough grouped slide material.
    """
    items = [x for x in ensure_list(hierarchy.get("outline_items")) if isinstance(x, dict) and x.get("role") != "skip"]
    if not items:
        return False
    levels = [int(x.get("level") or 1) for x in items]
    has_parent_child = any(x.get("parent_id") for x in items)
    has_level1 = any(l <= 1 for l in levels)
    has_deep_level = max(levels or [1]) >= 3
    # Enough top-level material or an explicit parent-child hierarchy is needed.
    return bool(has_parent_child or (has_level1 and has_deep_level) or len(set(levels)) >= 3)


def _merge_singleton_groups(groups: List[Dict[str, Any]], min_slides: int = MIN_SLIDES_PER_SECTION) -> List[Dict[str, Any]]:
    """Merge section groups that would create a one-slide section.

    Policy:
    - Never keep an isolated visual section with fewer than min_slides, unless the
      whole deck itself has fewer than min_slides slides.
    - Merge singleton groups into the previous group when possible; otherwise into
      the next group. This preserves reading order and avoids section fragmentation.
    """
    if not groups:
        return []
    total = sum(len(g.get("slides", [])) for g in groups)
    if total < min_slides:
        return groups

    merged: List[Dict[str, Any]] = []
    for group in groups:
        slides = list(group.get("slides", []))
        if len(slides) >= min_slides:
            merged.append({**group, "slides": slides})
            continue
        if not slides:
            continue
        if merged:
            merged[-1]["slides"].extend(slides)
            merged[-1].setdefault("merged_from", []).append(group.get("section_title"))
        else:
            # Temporarily keep it; it will be merged into the next non-empty group below.
            merged.append({**group, "slides": slides, "_needs_forward_merge": True})

    # If the first group was a singleton and there is a following group, merge forward.
    if len(merged) >= 2 and merged[0].get("_needs_forward_merge"):
        first = merged.pop(0)
        merged[0]["slides"] = first.get("slides", []) + merged[0].get("slides", [])
        merged[0].setdefault("merged_from", []).insert(0, first.get("section_title"))
    for g in merged:
        g.pop("_needs_forward_merge", None)
    return merged
...
def apply_section_policy(
    ...
    """Consolidate LLM groups into natural deck sections.

    LLM may propose many groups. This function is the deterministic safeguard:
    only keep multiple sections if the source outline is deep enough and after
    merging every section has at least MIN_SLIDES_PER_SECTION slides.
    """
    grouped = [g for g in grouped if g.get("slides")]
    total_slides = sum(len(g.get("slides", [])) for g in grouped)
    original_count = len(grouped)
    depth_ok = _outline_has_real_depth(hierarchy)

    if total_slides == 0:
        return [], {
            "enabled_multiple_sections": False,
            "reason": "no_slides",
            "original_group_count": original_count,
            "final_group_count": 0,
            "total_slides": 0,
        }
    ...
    if (
        not depth_ok
        or total_slides < MIN_TOTAL_SLIDES_FOR_MULTIPLE_SECTIONS
        or original_count < MIN_GROUPS_FOR_SECTION_HEADERS
    ):
        all_slides: List[Dict[str, Any]] = []
        for g in grouped:
            all_slides.extend(g.get("slides", []))
        return [
            {
                "group_key": "G_MAIN",
                "section_title": "Nội dung chính" if total_slides > 1 else safe_text(deck_title, "Nội dung chính"),
                "slides": all_slides,
            }
        ], {
            "enabled_multiple_sections": False,
            "reason": "outline_not_deep_enough_or_too_few_slides",
            "depth_ok": depth_ok,
            "original_group_count": original_count,
            "final_group_count": 1,
    ...
    merged = _merge_singleton_groups(grouped, MIN_SLIDES_PER_SECTION)
    if len(merged) <= 1:
        if merged:
            merged[0]["section_title"] = "Nội dung chính" if total_slides > 1 else merged[0].get("section_title", "Nội dung chính")
        return merged, {
            "enabled_multiple_sections": False,
            "reason": "groups_collapsed_after_singleton_merge",
            "depth_ok": depth_ok,
            "original_group_count": original_count,
            "final_group_count": len(merged),
            "total_slides": total_slides,
            "min_slides_per_section": MIN_SLIDES_PER_SECTION,
        }

    return merged, {
        "enabled_multiple_sections": True,
        "reason": "outline_deep_enough_and_each_section_has_enough_slides",
        "depth_ok": depth_ok,
        "original_group_count": original_count,
        "final_group_count": len(merged),
        "total_slides": total_slides,
        "min_slides_per_section": MIN_SLIDES_PER_SECTION,
        "final_groups": [{"section_title": g.get("section_title"), "slide_count": len(g.get("slides", [])), "merged_from": g.get("merged_from", [])} for g in merged],
    }
...
def aggregate_deck(state: PipelineState) -> PipelineState:
    ...
    grouped, section_policy_report = apply_section_policy(
        grouped,
        state.get("outline_hierarchy", {}),
        state["task_plan"].get("deck_title", "Báo cáo tổng hợp"),
    )
    ...
    state.setdefault("validation_reports", []).append(
        {
            "stage": "aggregate_by_llm_outline_groups",
            "groups": [{"section_title": g["section_title"], "slide_count": len(g["slides"]), "merged_from": g.get("merged_from", [])} for g in grouped],
            "section_policy": section_policy_report,
        }
    )
    return state
