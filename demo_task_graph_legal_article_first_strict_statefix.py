...
presentation "{{ title }}":
  meta:
    author: "Legal Article First State-Safe Pipeline"
    language: vi
    format: pptx
...
def node_pop_task(state: PipelineState) -> PipelineState:
    """Pop exactly one task without mutating the list in-place.

    LangGraph state can be copied/merged between node calls. In-place mutation
    of nested lists is fragile and was the reason the graph sometimes reached
    aggregation with an empty `completed_tasks`, causing the fallback slide to be
    generated even though `task_plan.json` contained all Điều/Phụ lục tasks.
    Always assign fresh list objects back into state.
    """
    pending = list(state.get("pending_tasks") or [])
    if pending:
        state["current_task"] = pending[0]
        state["pending_tasks"] = pending[1:]
    else:
        state["current_task"] = {}
        state["pending_tasks"] = []
    return state
...
def node_validate_repair_task(state: PipelineState) -> PipelineState:
    """Validate/repair one task and persist it without in-place state mutation."""
    spec = dict(state.get("current_task_output") or {})
    ...
    if errors:
        repair_log.append(f"{task.get('task_id')}: {errors}")
    ...
    if spec.get("status") == "completed":
        completed_tasks.append(spec)
    else:
        validation_reports.append({"stage": "task_skipped", "task_id": task.get("task_id"), "reason": spec.get("reason")})
...
def node_aggregate_deck(state: PipelineState) -> PipelineState:
    ...
    # Last-resort state-safe reconstruction: if LangGraph/state execution somehow
    # arrives here with no completed slides but a non-empty task plan, synthesize
    # slide specs deterministically from the task plan. This prevents the old bug
    # where the deck fell back to only the first few lines of Điều 1.
    if not completed and state.get("task_plan", {}).get("tasks"):
        reconstructed: List[Dict[str, Any]] = []
        for task in state.get("task_plan", {}).get("tasks", []):
            spec = create_slide_spec_from_task(task)
            if spec.get("status") == "completed" and not validate_slide_spec(spec):
                reconstructed.append(spec)
        if reconstructed:
            completed = reconstructed
            state["completed_tasks"] = completed
            state.setdefault("validation_reports", []).append({
                "stage": "aggregate_reconstruct_completed_tasks",
                "reason": "completed_tasks_empty_but_task_plan_nonempty",
                "reconstructed_slides": len(reconstructed),
            })
