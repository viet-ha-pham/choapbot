#!/usr/bin/env python3
"""
Task-graph document-outline-first PDF/DOCX/TXT/PLAIN-TEXT -> slide deck pipeline
======================================================
...
validates/repairs each task result, skips empty/unusable report components, and only then aggregates slide fragments into
one PML deck.

Core idea
---------
    PDF/DOCX/TXT report text
      -> document outline guides section-level summarization; ontology is only a reference/fallback
...
def strip_vietnamese_admin_noise(text: str, ontology: Optional[Dict[str, Any]] = None) -> str:
    ...
    Typical Vietnamese official documents start with the national title/motto and
    end with a recipient list ("Nơi nhận"). Those lines are metadata/protocol,
    not report substance, so the outline extractor should ignore them.
    """
    ...
    for raw in lines:
        ...
        if in_recipients_tail:
            # Usually at the end of official documents. Drop all following recipient/signature lines.
            continue
...
def summarize_report_by_ontology(state: PipelineState) -> PipelineState:
    """Summarize by the document's own outline first; use ontology only as reference.

    Older versions iterated through ontology concepts and asked the LLM to fill each
    concept. That made decks feel forced and could create slides not present in the
    source. This version extracts the source headings first, then asks Gemini to
    summarize each detected section while preserving the original heading order.
    """
    ...
    mock_summary = mock_summary_from_outline(outline, ontology)

    prompt = f"""
Bạn là chuyên gia tóm tắt văn bản hành chính để tạo slide.

NGUYÊN TẮC BẮT BUỘC:
1. Bám theo các đề mục thật đã phát hiện trong văn bản, giữ đúng thứ tự nguồn.
2. Không tự tạo slide theo ontology nếu văn bản không có mục/nội dung tương ứng.
3. Ontology chỉ là tham khảo để gán intent, chuẩn hóa cách hiểu và chọn layout khi cần.
4. Heading đầu ra nên giữ gần với source_heading; không thay bằng preferred_heading của ontology trừ khi source_heading quá mơ hồ.
5. Nếu một đề mục không có nội dung có nghĩa, bỏ qua.
6. Không tạo subtitle nếu subtitle chỉ là metadata như source heading, task id, layout, ontology, renderer.
7. Không dùng dấu ba chấm hoặc ký tự … để cắt ý; hãy viết câu/bullet hoàn chỉnh, ngắn gọn.
8. Bỏ qua quốc hiệu/tiêu ngữ mở đầu và mục "Nơi nhận" cuối văn bản.

Detected document outline:
{json.dumps(outline, ensure_ascii=False, indent=2)[:20000]}

Reference ontology concepts, optional only:
...
{{
  ...
  "sections": [
    {{
      ...
      "bullets": ["ý chính trích/tóm tắt từ đúng đề mục này"],
...
"""
    ...
    state["report_summary"] = normalize_summary(summary, state["report_ontology"], outline)
    skipped = state["report_summary"].get("skipped_sections", [])
    state.setdefault("validation_reports", []).append(
        {
            "stage": "document_outline_first",
            "detected_sections": [x.get("source_heading") for x in outline],
            "note": "Ontology was used only to label intent/concept; slide tasks follow source headings.",
        }
    )
    if skipped:
        state.setdefault("validation_reports", []).append(
...
def decompose_into_slide_tasks(state: PipelineState) -> PipelineState:
    ...
    state["task_plan"] = {
        ...
        "structure_source": summary.get("structure_source", "document_outline_first"),
