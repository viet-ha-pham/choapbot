#!/usr/bin/env python3
"""
Clean ontology-role LangGraph slide pipeline
===========================================
...
--------------
    input text / PDF / DOCX
      ...
      -> infer presentation intent with presentation ontology
      -> choose layout from layout registry
      -> summarize each task and create slide spec
...
presentation "{{ title }}":
  meta:
    author: "Clean Ontology Roles Pipeline"
    language: vi
    format: pptx
...
def strip_admin_boilerplate(text: str, report_ontology: Dict[str, Any]) -> str:
    ...
    for raw in lines:
        ...
        if in_tail:
            continue
        # If a line contains document code + motto in same row, drop it as boilerplate.
        if re.search(r"\bsố\s*[:：]?", line, re.I) and re.search(r"cộng\s+h[oòóọõỏô]", line, re.I):
            continue
...
def split_by_source_outline(text: str) -> List[Dict[str, Any]]:
    ...
    return [e for e in entries if not is_blank_or_placeholder(e.get("content", ""))]
...
def create_slide_spec_from_task(task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if first_sentence and len(first_sentence) <= 90 and first_sentence.lower() not in heading.lower():
        # Still conservative. Avoid subtitles that are just metadata.
        if not re.search(r"\b(task|layout|ontology|level|parent|theo đề mục)\b", first_sentence, re.I):
...
def node_extract_outline(state: PipelineState) -> PipelineState:
    out_dir = ensure_dir(state["out_dir"])
    outline = split_by_source_outline(state.get("cleaned_text", ""))
    state["source_outline"] = outline
    path = out_dir / "source_outline.json"
...
def node_create_task_plan(state: PipelineState) -> PipelineState:
    ...
    task_plan = {"tasks": tasks, "policy": {"skip_empty": True, "one_task_per_source_unit": True}}
...
def node_aggregate_deck(state: PipelineState) -> PipelineState:
    ...
    if not completed:
        section_groups = [{
            ...
            "slides": [{
                "title": "Không có nội dung đủ điều kiện",
                "subtitle": "",
                "intent": "summarize_core_message",
                "layout": "title-bullets",
                "blocks": {"bullets": ["Không tìm thấy mục nội dung đủ rõ sau khi loại bỏ phần hành chính/boilerplate."]},
            }],
        }]
    ...
    outputs = render_outputs(state["renderer_path"], pml_text, psl_text, out_dir)
...
def build_graph():
    ...
    graph.add_node("extract_outline", node_extract_outline)
    ...
    graph.add_edge("clean_text", "extract_outline")
    graph.add_edge("extract_outline", "classify_with_report_ontology")
    ...
    graph.add_edge("aggregate_deck", END)
    return graph.compile()


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Clean ontology-role task graph slide generator")
    ...
    parser.add_argument("--out-dir", default="out_clean_roles", help="Output directory")
