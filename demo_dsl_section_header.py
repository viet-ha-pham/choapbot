...
def parse_section(lines: List[Line], start: int) -> Tuple[Section, int]:
    ...
    while i < len(lines):
        ...
        if key == "header_layout":
            ...
            # Setting header to {} means: create a section header slide
            # even if no explicit header block is provided.
        ...
        if key == "header":
            if val not in ("", None):
                raise SyntaxError(f"Line {line.lineno}: header must be a block")
            if i + 1 >= len(lines) or lines[i + 1].indent <= line.indent:
                section.header = {}
                i += 1
            else:
                block, new_i = parse_generic_block(lines, i + 1, lines[i + 1].indent)
                if not isinstance(block, dict):
                    raise SyntaxError(f"Line {line.lineno}: header must be a mapping")
        ...
        if slide_match:
            ...
            raise SyntaxError(
                f"Line {line.lineno}: section only accepts header/header_layout/slide blocks"
            )
...
def parse_psl(text: str) -> Theme:
    ...
    while i < len(lines):
        ...
        if key == "page":
            ...
            # keep unknown blocks as slide layouts/extensions if desired
...
def build_render_ir(doc: PresentationDoc, theme: Theme) -> Dict[str, Any]:
    ...
    for section in doc.sections:
        if section.header is not None:
            ...
            header_objects = make_section_header_objects(
            ...
            slides_ir.append(
                {
                    ...
                    "notes": section.header.get("notes", "") if section.header else "",
                }
            )
...
def make_section_header_objects(
    section_title: str,
    header: Dict[str, Any],
    style: Dict[str, Any],
    theme: Theme,
    ...
    """
    Build render objects for a section divider/header slide.

    Supported header fields:
      title: optional override of section title
      subtitle: optional subtitle
      bullets: optional short agenda/list
    """
    objects: List[Dict[str, Any]] = []

    title_value = header.get("title", section_title)
    title_style = style.get("title", {})
    objects.append(make_text_object("SectionTitle", title_value, title_style, theme))

    if "subtitle" in header:
        subtitle_style = style.get("subtitle", {})
        objects.append(make_text_object("SectionSubtitle", header["subtitle"], subtitle_style, theme))

    if "bullets" in header:
        bullets_style = style.get("bullets", {})
        objects.append(make_bullet_object(header["bullets"], bullets_style, theme))
...
presentation "AI Strategy 2026":
  ...
  section "Bối cảnh":
    ...
    header:
      subtitle: Tổng quan vấn đề và mục tiêu
