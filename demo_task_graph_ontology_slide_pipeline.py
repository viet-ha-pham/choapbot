#!/usr/bin/env python3
"""
Task-graph ontology-guided PDF -> slide deck pipeline
======================================================
...
Mock run
--------
    python demo_task_graph_ontology_slide_pipeline.py dummy.pdf \
...
----------
    ...
    python demo_task_graph_ontology_slide_pipeline.py input.pdf \
      --renderer demo_dsl_conclusion_box.py \
      --out-dir out_task_graph_demo
...
def extract_pdf_text(pdf_path: Path, mock: bool) -> str:
    if mock:
        return (
    ...
    try:
        ...
        raise RuntimeError("For local PDF text extraction, install pypdf or use --mock / Gemini File API.")
...
def summarize_report_by_ontology(state: PipelineState) -> PipelineState:
    source_text = extract_pdf_text(Path(state["pdf_path"]), state.get("mock", False))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Task-graph ontology slide generation pipeline")
    parser.add_argument("pdf_path", help="Input PDF path. In --mock mode this can be any placeholder path.")
...
def main() -> None:
    ...
    print("Generated:")
    for key in ["pml_path", "psl_path", "task_plan_path", "html_path", "pptx_path"]:
        if final.get(key):
