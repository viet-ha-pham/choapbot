...
DEFAULT_PIPELINE_PATH = APP_DIR / "demo_task_graph_long_report_scalable.py"
...
def main() -> None:
    ...
    st.caption("Input báo cáo dài → chunk/task graph → PML/PSL → HTML/PPTX/Markdown. Có chọn style và tham số chunk/task.")
    ...
    with st.sidebar:
        ...
        max_chunk_chars = st.number_input(
            ...
            value=12_000,
            step=1_000,
            help="Giới hạn độ dài mỗi chunk văn bản trước khi trích outline/tóm tắt. Report càng dài thì nên giữ 8k–15k.",
        )
        max_task_chars = st.number_input(
            ...
            value=7_000,
            step=500,
            help="Giới hạn nội dung mỗi task slide. Nếu một đề mục quá dài, pipeline sẽ tách thành nhiều task nhỏ.",
        )
    ...
    try:
        ...
        st.caption(f"Long-report params: max_chunk_chars={int(max_chunk_chars):,} | max_task_chars={int(max_task_chars):,}")
