...
def normalize_bullet_items(value: Any, default_icon: str = "") -> List[Dict[str, str]]:
    ...
    for raw in raw_items:
        if isinstance(raw, dict):
            ...
            items.append({"icon": icon, "text": str(text)})
        else:
            items.append({"icon": default_icon, "text": str(raw)})
    return items
...
def make_stair_progress_objects(slide_blocks: Dict[str, Any], style: Dict[str, Any], theme: Theme) -> List[Dict[str, Any]]:
    ...
    for idx, step in enumerate(steps):
        ...
        objects.append({
            ...
            "w": step_w,
            "h": step_h,
            "fill": style_value(stair_style, "fill", theme, "#FFFFFF"),
            "border": style_value(stair_style, "border", theme, theme.colors.get("primary", "#2563EB")),
        })
...
def make_stacked_stairs_objects(slide_blocks: Dict[str, Any], style: Dict[str, Any], theme: Theme) -> List[Dict[str, Any]]:
    ...
    for idx, step in enumerate(steps):
        ...
        objects.append({
            ...
            "w": w,
            "h": h,
            "fill": style_value(stair_style, "fill", theme, theme.colors.get("secondary", "#E6F0FF")),
            "border": style_value(stair_style, "border", theme, theme.colors.get("primary", "#2563EB")),
        })
...
def apply_pptx_text_style(paragraph, obj: Dict[str, Any]) -> None:
    ...
    paragraph.font.size = Pt(obj["size"])
...
def add_textbox(slide, obj: Dict[str, Any], bullet: bool = False) -> None:
    ...
    if bullet:
        for idx, item in enumerate(obj["items"]):
            ...
            p.text = bullet_item_to_plain_text(item)
            p.level = 0
            apply_pptx_text_style(p, obj)
    else:
        p = tf.paragraphs[0]
        p.text = obj["text"]
        apply_pptx_text_style(p, obj)
...
.textbox, .bullets, .image, .card, .linebox, .circle, .bg-image { position: absolute; box-sizing: border-box; }
                ...
                for item in obj["items"]:
                    ...
                    if icon:
                        parts.append(
                            ...
                            f"<span style='color:{icon_color}; font-size:{icon_size}px; line-height:1.2; min-width:{icon_size}px;'>"
                            f"{icon}</span><span style='line-height:1.25;'>{text}</span></div>"
                        )
                    else:
                        parts.append(f"<div style='margin-bottom:{line_gap}px; line-height:1.25;'>• {text}</div>")
                parts.append("</div>")
            elif obj["type"] == "Image":
                mode = normalize_fit_mode(obj.get("mode", "stretch"))
                fit_map = {"stretch": "fill", "contain": "contain", "cover": "cover", "original": "none"}
