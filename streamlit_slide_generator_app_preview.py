...
Run:
    ...
    streamlit run streamlit_slide_generator_app.py

Recommended renderer:
    demo_dsl_markdown_eval.py
"""
...
DEFAULT_PIPELINE_PATH = APP_DIR / "demo_task_graph_section_policy.py"
DEFAULT_RENDERER_PATH = APP_DIR / "demo_dsl_inline_markdown_styles.py"
DEFAULT_REPORT_ONTOLOGY_RICH = APP_DIR / "report_domain_ontology_admin_clean.json"
DEFAULT_SLIDE_ONTOLOGY_RICH = APP_DIR / "slide_creation_ontology_rich.json"
DEFAULT_LAYOUT_REGISTRY = APP_DIR / "layout_registry.json"
...
def main() -> None:
    ...
    st.title("📊 Ontology + LangGraph Slide Generator")
    st.caption("Input báo cáo → task graph → PML/PSL → HTML/PPTX/Markdown. Có chọn style bằng selectbox.")

    with st.sidebar:
        st.header("Cấu hình")
        style_name = st.selectbox("Style / Theme", list(STYLE_PRESETS.keys()), index=0)
        mock = st.checkbox("Mock mode, không gọi Gemini", value=True)
        ...
        slide_ontology_upload = st.file_uploader("Slide ontology JSON", type=["json"], key="slide_ontology")
    ...
    with col_b:
        st.info("Gợi ý: dùng `demo_dsl_markdown_eval.py` để có thêm Markdown phục vụ G-Eval.")
    ...
    try:
        ...
        slide_ontology_path = save_uploaded_json(
            slide_ontology_upload,
            out_dir,
            DEFAULT_SLIDE_ONTOLOGY_RICH,
        ...
        with st.spinner("Đang chạy LangGraph pipeline..."):
            ...
            initial_state = {
                ...
                "slide_ontology_path": slide_ontology_path,
                "layout_registry_path": layout_registry_path,
            }
            final_state = graph.invoke(initial_state)

        # Apply selected visual style after task planning, then rerender all outputs.
        ...
        with st.spinner("Đang áp dụng style đã chọn và render HTML/PPTX/Markdown..."):
